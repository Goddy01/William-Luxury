...
import { getCharterBySlug, formatEur, charterListings } from "@/data/charter";
...
export function generateStaticParams() {
  return charterListings
    .filter((c) => c.hasDetailPage)
    .flatMap((c) =>
      ["it", "en"].map((locale) => ({ locale, slug: c.slug }))
    );
}
...
export async function generateMetadata({
  ...
  const boat = getCharterBySlug(slug);
...
export default async function CharterDetailPage({
  ...
  const loc = locale as "it" | "en";
  const price = `${formatEur(boat.prices.day, locale)}${t("perDay")}`;
          ...
          <div className="min-w-0 lg:col-span-8">
            ...
            <div className="mt-8 border border-brass/30 bg-navy p-6 text-cream sm:p-8 lg:hidden">
              ...
              {boat.captainIncluded && (
                <p className="mt-2 text-sm text-cream/60">{t("captain")}</p>
              )}
              <Link
